...
def fetch_pressure_today(condition :str):
    client = MongoClient(MONGO_URI)
    db = client[DB_NAME]
    collections = [c for c in db.list_collection_names() if c.startswith("pressure")]
...
def fetch_pressure_weekly(condition: str):
...
def fetch_pressure_monthly(condition: str):
    client = MongoClient(MONGO_URI)
    db = client[DB_NAME]
    collections = [c for c in db.list_collection_names() if c.startswith("pressure")]
